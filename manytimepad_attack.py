
ct1 = ""
ct2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
ct3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
ct4 = ""
ct5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
ct6 = ""
ct7 = ""
ct8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
ct9 = ""
ct10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"

target = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

ciphertexts = [ct1, ct2, ct3, ct4, ct5, ct6, ct7, ct8, ct9, ct10]

n_ciphertexts = 10


# From code used to generate the ciphertexts provided on the assignment website.
def strxor(a, b):
    return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b)])


def main():
    letters = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
    decoded_ciphertexts = [''.join([chr(int(''.join(c), 16)) for c in zip(ct[0::2], ct[1::2])]) for ct in ciphertexts]
    decoded_target = ''.join([chr(int(''.join(c), 16)) for c in zip(target[0::2], target[1::2])])

    # individual ciphertexts XORed with the target ciphertext
    cts_xored_w_target = [strxor(decoded_target, ct) for ct in decoded_ciphertexts]

    # starting default guess (i.e., we know nothing at the start of the process)
    msg_guess = ["-"] * len(decoded_target)

    list_of_space_guesses_per_ct = []

    for ciphertext in decoded_ciphertexts:
        ciphertext_length = len(ciphertext)
        spaces_guess = ["-"] * ciphertext_length

        # ciphertexts XORed with one another
        cts_xored_w_each_other = [strxor(ciphertext, diff_ciphertext) for diff_ciphertext in decoded_ciphertexts if
                                  diff_ciphertext != ciphertext]

        for idx in range(ciphertext_length):
            space_counter = 0
            for xor in cts_xored_w_each_other:
                # Note that xor[idx] == '\x00' evaluates to True when the character of ciphertext x and y matched at
                # that index. Let us assume that if xor[idx] == '\x00' evaluates to true, both ciphertexts have a
                # space there. Therefore, if xor[idx] != '\x00' evaluates to true and xor[idx] is not a letter in the
                # alphabet, then there is no space marker there in our ciphertext. Otherwise, there is. Let's keep
                # track of the number of spaces we think we have in our ciphertext.
                if idx < len(xor) and xor[idx] != '\x00' and xor[idx] not in letters:
                    continue
                else:
                    space_counter += 1
            # If we see that xor[idx] == '\x00' evaluated to true in at least 7 different pairings of the
            # ciphertexts, then we should be pretty confident that there is a space at this index in this particular
            # ciphertext.
            if space_counter > 7:
                spaces_guess[idx] = " "
        list_of_space_guesses_per_ct.append(spaces_guess)

    for idx in range(n_ciphertexts):
        ct_spaces = list_of_space_guesses_per_ct[idx]
        msg_xord_with_target = cts_xored_w_target[idx]
        ct_length = len(ct_spaces)
        for jdx in range(ct_length):
            if jdx < len(msg_guess) and ct_spaces[jdx] == " ":
                # If you know that the ciphertext had a space, then you know that the XOR of the ciphertext's and the
                # target's character would result in a copy of the target's character, with the opposite case.
                msg_guess[jdx] = msg_xord_with_target[jdx].swapcase()
    print("".join(msg_guess))

    #msg_guess = "The secret message is: When using a stream cipher, never use the key more than once"


if __name__ == "__main__":
    main()
